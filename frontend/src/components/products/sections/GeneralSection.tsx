...
export const GeneralSection: React.FC<GeneralSectionProps> = ({ formData, setFormData, onChange, industryFields = [] }) => {
  
  const generalIndustryFields = industryFields.filter(f => {
  ...
  const activeFields = generalIndustryFields.filter(f => formData.customFields?.[f.name] !== undefined);
    ...
    <Box sx={{ p: 2.5, bgcolor: 'white', borderRadius: 3, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
      ...
      <Grid container spacing={2.5}>
        ...
        <Grid size={{ xs: 12, md: 3 }}>
          ...
          >
            {['Active', 'Inactive', 'Draft', 'Out Of Stock'].map((status) => (
